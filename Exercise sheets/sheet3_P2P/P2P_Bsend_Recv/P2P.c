...
int main(int argc, char** argv) {
    ...
    char *buffer, *bsend_buffer;
    int bsend_buffer_size;

    // Calculate the size for the MPI buffered send
    bsend_buffer_size = MPI_BSEND_OVERHEAD + sizes[num_sizes - 1];
    ...
    // Detach and free the buffer for MPI_Bsend
