...
void quicksort_distributed_recursive(float pivot, int start, int end, float* &data,  MPI_Comm comm)
{
    ...
    {
        quicksort_recursive(lower_pivot,start,belows_end, data);
        quicksort_recursive(upper_pivot,aboves_start,end, data);
    }
    else
    {
        ...
        {
            quicksort_distributed_recursive(lower_pivot, start, belows_end, data,  new_comm);
        ...
        {
            quicksort_distributed_recursive(upper_pivot, aboves_start, end, data, new_comm);
...
void quicksort_distributed(float pivot, int start, int end, float* &data, MPI_Comm comm)
{
        ...
        //Start the sorting
        quicksort_distributed_recursive(pivot,start,end,data,comm);
        cudaDeviceSynchronize();
